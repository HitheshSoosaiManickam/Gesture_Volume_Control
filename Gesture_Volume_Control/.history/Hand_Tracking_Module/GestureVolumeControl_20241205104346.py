...
while True:
        ...
        if len(lmList)!=0:
             ...
             vol = np.interp(length, [22,200], [minVol,maxVol])
             volBar = np.interp(length, [22,200], [400,150])
             volPer = np.interp(length, [22,200], [0,100])
        ...
        cv2.putText(img, f"Vol: {str(int(volPer))}%", (40,450),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0,0,0), thickness=2)
